...
class PlayerCareerScore extends React.Component {
	...
	render() {
		const options = {
			title: {
				text: this.props.store.playerData.playerFirstName + " " + this.props.store.playerData.playerLastName + '\'s Career ' + TITLE_MAP[this.state.tab] + ' Board'
			},
		...
		return (
			<div>
				<div>
					<Select defaultValue={this.state.tab} style={{ paddingLeft: 20, width: 100 }} onChange={this.onTabChange}>
						{
							TABS.map(t => (
